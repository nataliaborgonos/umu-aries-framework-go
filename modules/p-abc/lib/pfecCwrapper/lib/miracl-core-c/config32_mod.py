...
def usage():
    print("Usage: ./config32.py [OPTIONS] [ARGUMENTS]\n")
    ...
    print("   -o               --options=1 --options=32 Disable interactive mode and select specific options")
    ...
    for opt, arg in opts:
        if opt in ['-h', '--help']:
            ...
            try:
                ...
                print("config32.py: Error! You have passed an invalid option, exiting.")
                print("             If you are trying to select multiple options, please repeat the flag\n")
                print("             e.g. ")
                print("                  ./config32_mod.py -o 1 -o 23 -o 25")
                sys.exit(1)
...
def main(argv):
    ...
    if not arg_manager(argv, options_list):
        if intr:
            ...
            print("config32.py: Invalid input, program terminating")
            sys.exit(2)
    else:
